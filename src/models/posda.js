...
bcrypt.hash(password, 10, (err, encryptedPass) => {
    ...
    const loginPosda = (body) => {
      return new Promise((resolve, reject) => {
        ...
        const qs =
          "SELECT posda.id, posda.username, posda.name, posda.password,  posda.phone , posda.isVerify , posda.avatar , role.name AS role FROM posda JOIN role ON posda.role =role.id WHERE posda.username=?";
